...
LastEditTime: 2021-05-27 13:57:13
LastEditors: DJ
'''
import random
...
class GameControl():
    ...
    def __init__(self,background,plane_palyer,list_enemy,list_bullet,str_name_player,path_player_img,list_background_img_path,max_x,max_y) -> None:
        ...
        self.crack_threshold=10 # 认定为碰撞的重叠度阈值，当重叠度大于阈值时才认为两个对象碰撞了
        # 使用定时器定时创建敌军
    ...
    def crack_detect(self):
        ...
        """检测敌方飞机与玩家子弹的碰撞"""
        for enemy in self.list_bullet:
            for bullet in [x for x in self.list_bullet if x.class_bullet=='palyer' ]
        pass
        

    '''
    @description: 检测两个对象的重叠度，如果不重叠返回0，如果重叠返回 整数类型重叠度
    @param  {*}
    @return {*}
    @param {*} self
    ...
    def crack_detect_object(self,object_a,object_b):
        ...
        if x1>x2+w2 or y1>y2+h2 or x1+w1<x2 or y1+h1<y2:
            ...
            area1 = w1 * h1
            area2 = w2 * h2
            overlap_percent= int(overlap_area / (area1 + area2 - overlap_area)*100)
