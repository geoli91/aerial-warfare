...
LastEditTime: 2021-05-27 11:42:14
LastEditors: DJ
'''
...
class PlaneEnemy(BasePlane):
    def __init__(self,class_plane,class_bulet,list_bullet,list_enemy,max_x,max_y) -> None:
        ...
        self.y=0
        # 创建定时器，定时移动敌机
        self.qtimer_move_=QtCore.QTimer()
        self.qtimer.start(200)
        self.qtimer.timeout.connect(self.move_enemy)
    ...
    def move_enemy(self):
        print("move_enemy")
        dict_int_move={1:self.move_left,2:self.move_right,3:self.move_up,4:self.move_low}
        func_int_move=lambda int_i:dict_int_move.get(int_i,self.move_low)()
        int_i=random.randint(1,4)
        for i in range(0,3):
            func_int_move(int_i)
