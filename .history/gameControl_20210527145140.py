...
LastEditTime: 2021-05-27 14:51:39
LastEditors: DJ
'''
import random
...
class GameControl():
    ...
    """将 int_score 设置为属性，以便使用 setter 监控分数，大于等于1000分时进入下一关"""
    @property
    def int_score(self):
        ...
        for enemy in self.list_enemy:
            for bullet in [x for x in self.list_bullet if x.class_bullet=='player' ]:
                ...
                if overlap_percent>self.crack_threshold:
                    ...
                    bullet.remove_bullet()
        """检测玩家子弹与敌方子弹的碰撞"""
        for bullet_enemy in [x for x in self.list_bullet if x.class_bullet=='enemy' ]:
            for bullet_player in [x for x in self.list_bullet if x.class_bullet=='player' ]:
                overlap_percent=self.crack_detect_object(bullet_enemy,bullet_player)
                if overlap_percent>self.crack_threshold:
                    bullet_enemy.remove_bullet()
                    bullet_player.remove_bullet()
