...
LastEditTime: 2021-05-27 16:24:30
LastEditors: DJ
'''
...
class PlanePlayer(BasePlane):
    ...
    '''    
    def move(self):
        if self.keyboard.is_key_down(QtCore.Qt.Key_W) or self.keyboard.is_key_down(QtCore.Qt.key_ip):
            self.move_up()
        if self.keyboard.is_key_down(QtCore.Qt.Key_S):
            self.move_low()
        if self.keyboard.is_key_down(QtCore.Qt.Key_A):
            self.move_left()
