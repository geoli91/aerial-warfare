...
LastEditTime: 2021-05-27 16:24:48
LastEditors: DJ
'''
...
class BasePlane(QtCore.QObject):
    ...
    def bomb(self):
        ...
        path_img_bomb=None
        # 如果是玩家飞机，使用一种图标
        if self.class_plane=='player':
            path_img_bomb=r'resources\photo\bomb4.png'
        # 如果是敌军飞机，随机选择一种图标
        else:
            dict_int_path={
                1:r'resources\photo\bomb0.png',
                2:r'resources\photo\bomb1.png',
                3:r'resources\photo\bomb2.png',
                4:r'resources\photo\bomb3.png',
                5:r'resources\photo\bomb5.png'
                }
            func_int_path=lambda class_plane:dict_int_path.get(class_plane,r'resources\photo\bomb5.png')
            path_img_bomb=func_int_path(random.randint(1,5))
        # path_img_bomb=r'resources\photo\bomb5.png'
...
class PlanePlayer(BasePlane):
    ...
    def move(self):
        if self.keyboard.is_key_down(QtCore.Qt.Key_W) or self.keyboard.is_key_down(QtCore.Qt.Key_Up):
            self.move_up()
        if self.keyboard.is_key_down(QtCore.Qt.Key_S) or self.keyboard.is_key_down(QtCore.Qt.Key_Up):
            self.move_low()
        if self.keyboard.is_key_down(QtCore.Qt.Key_A) or self.keyboard.is_key_down(QtCore.Qt.Key_Up):
            self.move_left()
        if self.keyboard.is_key_down(QtCore.Qt.Key_D) or self.keyboard.is_key_down(QtCore.Qt.Key_Up):
            self.move_right()
        # 如果左键和右键都没有按下，还原飞机方向
        if not self.keyboard.is_key_down(QtCore.Qt.Key_W) and not self.keyboard.is_key_down(QtCore.Qt.Key_D) :
...
class PlaneEnemy(BasePlane):
    ...
    '''
    @description: 重写敌方飞机的 bomb 事件，
    @param  {*}
    @return {*}
    @param {*} self
    '''    
    def bomb(self):
        # 如果爆炸的是敌军飞机，使用Qtimer，爆炸几秒后，移除飞机
        qtimer=QtCore.QTimer()
        qtimer.singleShot(200,self.remove_enemy)
        return super().bomb()