...
LastEditTime: 2021-05-27 11:18:32
LastEditors: DJ
'''
import sys
...
class FormGame(QtWidgets.QDialog,Ui_FormGame):
    ...
    def init_game(self):
        ...
        # 实例化游戏控制对象
        self.gameControl=GameControl(self.background,self.plane_palyer,self.list_enemy,self.list_bullet,self.str_name_player,self.path_player_img,self.list_background_img_path,self.width())
        # 创建关卡
        self.gameControl.create_stage()
        # 创建定时器，定时刷新
        self.qtimer_update=QTimer() # 创建 QTimer 对象
        self.qtimer_update.start(100) # 开始定时，传入参数为时间间隔，单位为 毫秒
